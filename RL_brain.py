...
class RL(object):
    ...
    def check_state_exist(self, state):
        if state not in self.q_table.index:
         ...
         new_state_series = pd.Series(
            [0]*len(self.actions),
            index=self.q_table.columns,
            name=state,
         )
         # 使用pandas.concat代替append
         self.q_table = pd.concat([self.q_table, pd.DataFrame(new_state_series).T], ignore_index=False)
